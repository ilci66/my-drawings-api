...
let duplicates;
(async()=>{
  let titleArr = drawings.map(item => item.title);
  let isDublicate = await titleArr.some((url, i) => {
    return titleArr.indexOf(url) != i
  }) ;
  console.log(isDublicate)
  if(isDublicate){
    duplicate = await titleArr.find((value, index, self) => index !== self.findIndex((t) =>  t == value ) )
